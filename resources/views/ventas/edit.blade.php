...
<style>
    ...
    .vendedor-card:hover { border-color: #a08c78; background: #fff; box-shadow: 0 2px 6px rgba(160,140,120,0.12); }
    .vendedor-card.seleccionado { border-color: #2c2117; background: #f5f0eb; box-shadow: 0 2px 8px rgba(44,33,23,0.12); }

    .vendedor-avatar {
        width: 34px; height: 34px; border-radius: 50%;
        background: #2c2117; color: #f5f0eb;
        font-size: 13px; font-weight: 600;
        display: flex; align-items: center; justify-content: center;
        flex-shrink: 0;
    }

    .vendedor-card.seleccionado .vendedor-avatar { background: #6b5744; }
    .vendedor-nombre { font-size: 13px; font-weight: 500; color: #2c2117; line-height: 1.3; }

    .catalogo-search {
        width: 100%; padding: 8px 12px;
        border: 1px solid #d8cfc7; border-radius: 7px;
        background: #fdfaf7; font-size: 13px; font-family: inherit;
        color: #2c2117; outline: none;
    }

    .catalogo-search:focus { border-color: #a08c78; background: #fff; box-shadow: 0 0 0 3px rgba(160,140,120,0.15); }
    ...
    .art-card {
        background: #fdfaf7; border: 1px solid #e8e0d8;
        border-radius: 8px; padding: 10px; cursor: pointer;
        transition: border-color 0.15s, box-shadow 0.15s, background 0.15s;
        display: flex; flex-direction: column; gap: 6px;
    }

    .art-card:hover { border-color: #a08c78; background: #fff; box-shadow: 0 2px 8px rgba(160,140,120,0.15); }
    .art-card.en-carrito { border-color: #5a9a6a; background: #f0faf3; }

    .art-card img { width: 100%; aspect-ratio: 1; object-fit: cover; border-radius: 6px; background: #f5f0eb; }

    .art-card-img-placeholder {
        width: 100%; aspect-ratio: 1; border-radius: 6px;
        background: #f5f0eb; display: flex; align-items: center; justify-content: center; color: #c4b8ac;
    }
    ...
    .carrito-item-nombre {
        font-size: 12px; font-weight: 600; color: #2c2117;
        display: flex; align-items: flex-start; justify-content: space-between; gap: 6px;
    }

    .carrito-item-nombre button {
        background: none; border: none; cursor: pointer;
        color: #c4b8ac; padding: 0; flex-shrink: 0; transition: color 0.15s;
    }

    .carrito-item-nombre button:hover { color: #c0392b; }

    .carrito-item-inputs { display: grid; grid-template-columns: 1fr 1fr; gap: 6px; }

    .carrito-item-inputs label {
        font-size: 10px; font-weight: 500; color: #a08c78;
        margin-bottom: 3px; display: block;
    }

    .carrito-item-inputs input {
        width: 100%; padding: 5px 8px;
        border: 1px solid #d8cfc7; border-radius: 5px;
        background: #fff; font-size: 12px; font-family: inherit; color: #2c2117; outline: none;
    }

    .carrito-item-inputs input:focus { border-color: #a08c78; box-shadow: 0 0 0 2px rgba(160,140,120,0.12); }
    .carrito-item-inputs input[readonly] { background: #f5f0eb; color: #6b5744; cursor: default; }

    .carrito-item-subtotal { font-size: 11px; color: #6b5744; text-align: right; font-weight: 600; }

    .carrito-empty {
        display: flex; flex-direction: column; align-items: center;
        justify-content: center; gap: 8px; color: #c4b8ac;
        font-size: 12px; padding: 40px 0; flex: 1;
    }

    .carrito-footer {
        padding: 12px 16px; border-top: 1px solid #ede7df;
    ...
    .cliente-input:focus { border-color: #a08c78; background: #fff; box-shadow: 0 0 0 3px rgba(160,140,120,0.15); }
...
<div class="section-header">
    <h1>Editar venta #{{ $venta->numero }}</h1>
    <a href="{{ route('ventas.index', ['tab' => $venta->estado_id === 1 ? 'activas' : 'anuladas']) }}" class="btn btn-secondary">
...
@php
    ...
    $detallesExistentes  = $venta->detalles->map(fn($d) => [
        'id'       => (string) $d->articulo_id,
        'cantidad' => (float)  $d->cantidad,
        'precio'   => (float)  $d->precio_unitario,
    ])->values()->toArray();
@endphp

<form method="POST" action="{{ route('ventas.update', $venta) }}" id="form-venta">
@csrf @method('PUT')
    ...
    <div class="panel">
        ...
        <div class="vendedor-list">
            @foreach($vendedores as $v)
                ...
                <div class="vendedor-card {{ $venta->vendedor_id == $v->id ? 'seleccionado' : '' }}"
                     data-id="{{ $v->id }}" onclick="seleccionarVendedor(this)">
        ...
        <input type="hidden" name="vendedor_id" id="vendedor_id" value="{{ $venta->vendedor_id }}">
    ...
    <div class="panel">
        ...
        <div class="carrito-footer">
            <div>
                ...
                <input type="text" name="cliente_nombre" class="cliente-input"
                       value="{{ $venta->cliente_nombre }}" placeholder="Nombre del cliente…" maxlength="150">
            </div>
            <div class="total-row">
...
function agregarAlCarrito(card) {
    ...
    if (carrito.has(id)) {
        ...
        const info = articulosMap.get(id);
        carrito.set(id, { ...info, cantidad: 1 });
        renderItem(id);
    ...
    div.innerHTML = `
        ...
        <div class="carrito-item-inputs">
            ...
            <div>
                ...
                <input type="number" class="ci-precio" data-id="${id}"
...
document.getElementById('search-productos').addEventListener('input', function() { filtrar('grid-productos', this.value); });
document.getElementById('search-servicios').addEventListener('input', function() { filtrar('grid-servicios', this.value); });
...
document.getElementById('form-venta').addEventListener('submit', function(e) {
    ...
    if (carrito.size === 0) { e.preventDefault(); mostrarError('Agregá al menos un ítem.'); return; }
    let err = null;
    carrito.forEach(item => { if (item.servicio && item.precio <= 0) err = `Ingresá el precio para "${item.nombre}".`; });
...
(function() {
    detallesExistentes.forEach(d => {
        const info = articulosMap.get(d.id);
        if (!info) return;
        carrito.set(d.id, { ...info, cantidad: d.cantidad, precio: d.precio });
        renderItem(d.id);
        // Si es servicio, actualizar precio en el input
        if (info.servicio) {
            const inputP = document.querySelector(`.ci-precio[data-id="${d.id}"]`);
            if (inputP) inputP.value = d.precio;
        }
        const card = document.querySelector(`.art-card[data-id="${d.id}"]`);
