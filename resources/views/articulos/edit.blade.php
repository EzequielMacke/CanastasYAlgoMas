...
@php $backTab = $articulo->estado_id === 1 ? 'activos' : 'inactivos'; @endphp

<div class="section-header">
    <div>
        <h1>Editar artículo</h1>
        <p style="font-size:13px;color:#a08c78;margin-top:4px;">{{ $articulo->nombre }}</p>
    </div>
    <a href="{{ route('articulos.index', ['tab' => $backTab]) }}" class="btn btn-secondary">
    ...
    <form method="POST" action="{{ route('articulos.update', $articulo) }}" enctype="multipart/form-data">
        ...
        <div class="form-group">
            ...
            <div class="input-wrap">
                ...
                <input type="text" id="nombre" name="nombre" value="{{ old('nombre', $articulo->nombre) }}" required autofocus>
            </div>
        ...
        <div class="form-group">
            ...
            <textarea id="descripcion" name="descripcion" placeholder="Descripción opcional del artículo...">{{ old('descripcion', $articulo->descripcion) }}</textarea>
            ...
            <div class="form-group">
                ...
                <div class="input-wrap">
                    ...
                    <input type="number" id="precio_costo" name="precio_costo"
                           value="{{ old('precio_costo', $articulo->precioCosto?->costo) }}"
        ...
        <div class="form-group">
            ...
            <select id="unidad_medida_id" name="unidad_medida_id" required>
                ...
                @foreach($unidades as $unidad)
                    <option value="{{ $unidad->id }}" {{ old('unidad_medida_id', $articulo->unidad_medida_id) == $unidad->id ? 'selected' : '' }}>
        ...
        <div class="form-group">
            ...
            @if($articulo->foto)
                <img src="{{ asset('storage/' . $articulo->foto) }}"
                     alt="{{ $articulo->nombre }}"
                     class="foto-preview">
                <p style="font-size:12px;color:#a08c78;margin-top:6px;margin-bottom:10px;">
                    Subí una nueva imagen para reemplazarla.
                </p>
            @endif
        ...
        <div style="margin-top: 28px; display: flex; gap: 10px;">
            ...
            <a href="{{ route('articulos.index', ['tab' => $backTab]) }}" class="btn btn-secondary">Cancelar</a>
        </div>
    </form>
</div>
