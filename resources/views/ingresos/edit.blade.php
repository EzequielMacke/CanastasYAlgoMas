...
<style>
    ...
    .unidad-badge { display:inline-flex; align-items:center; gap:5px; background:#f5f0eb; color:#6b5744; border:1px solid #e8e0d8; border-radius:6px; padding:9px 14px; font-size:14px; font-weight:500; white-space:nowrap; }
...
@php $backTab = $ingreso->estado_id === 1 ? 'activos' : 'inactivos'; @endphp

<div class="section-header">
    <div>
        <h1>Editar ingreso</h1>
        <p style="font-size:13px;color:#a08c78;margin-top:4px;">{{ $ingreso->articulo->nombre }} — {{ $ingreso->fecha->format('d/m/Y') }}</p>
    </div>
    <a href="{{ route('ingresos.index', ['tab' => $backTab]) }}" class="btn btn-secondary">
    ...
    <form method="POST" action="{{ route('ingresos.update', $ingreso) }}">
                ...
                <select id="articulo_id" name="articulo_id" required style="flex:1;">
                    ...
                    @foreach($articulos as $art)
                        <option value="{{ $art->id }}"
                                ...
                                {{ old('articulo_id', $ingreso->articulo_id) == $art->id ? 'selected' : '' }}>
        ...
        <div id="precio-manual-wrap" class="form-group" style="margin-top:20px;">
            ...
            <div class="input-wrap">
                ...
                <input type="number" id="precio" name="precio"
                       value="{{ old('precio', $ingreso->precio) }}"
                       min="0" step="any">
            </div>
        ...
        <div class="form-group" style="margin-top:20px;">
            ...
            <textarea id="observacion" name="observacion"
                      placeholder="Proveedor, remito, notas...">{{ old('observacion', $ingreso->observacion) }}</textarea>
        ...
        <div style="margin-top:28px;display:flex;gap:10px;">
            ...
            <a href="{{ route('ingresos.index', ['tab' => $backTab]) }}" class="btn btn-secondary">Cancelar</a>
        </div>
    </form>
</div>
...
(function () {
    ...
    if (!esProduccion()) actualizarCalculo();
