                ...
                <div class="card shadow-sm border-0 rounded-4">
                    <div class="card-body p-4">
                        <div class="d-flex justify-content-between align-items-center mb-4">
                            <h3 class="text-primary fw-bold mb-0"><i class="bi bi-archive me-2"></i>Lista de Insumos</h3>
                            <div>
                                <a href="{{ route('insumo.create') }}" class="btn btn-success me-2">
                                ...
                                <tbody>
                                    @forelse($insumos as $insumo)
                                        <tr>
                                            ...
                                            <td>{{ $insumo->tipo ? $insumo->tipo->descripcion : '-' }}</td>
                                            <td>{{ $insumo->unidadMedida ? $insumo->unidadMedida->descripcion : '-' }}
                                                @if($insumo->unidadMedida && $insumo->unidadMedida->abreviacion)
                                                    ({{ $insumo->unidadMedida->abreviacion }})
                                                @endif
                                            </td>
                                            <td>{{ $insumo->unidadMedida ? $insumo->unidadMedida->correccion : '-' }}</td>
                                            <td>{{ $insumo->unidadMedida ? $insumo->unidadMedida->operacion_correccion : '-' }}</td>
                                            <td>{{ $insumo->estado ? $insumo->estado->descripcion : '-' }}</td>
                                        </tr>
                                    @empty
                                        <tr>
